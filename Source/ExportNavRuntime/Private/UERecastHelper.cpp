...
bool UERecastHelper::dtIsValidNavigationPoint(dtNavMesh* InNavMeshData, const UERecastHelper::FVector3& InPoint, const UERecastHelper::FVector3& InExtent)
{
	bool bSuccess = false;

	using namespace UERecastHelper;

	if (!InNavMeshData) return bSuccess;

	FVector3 RcPoint = UERecastHelper::Unreal2RecastPoint(InPoint);
	const FVector3 ModifiedExtent = InExtent;
	FVector3 RcExtent = UERecastHelper::Unreal2RecastPoint(ModifiedExtent).GetAbs();
	...
	if (PolyRef > 0)
	{
		const FVector3& UnrealClosestPoint = UERecastHelper::Recast2UnrealPoint(ClosestPoint);
...
int UERecastHelper::findStraightPath(dtNavMesh* InNavMeshData, dtNavMeshQuery* InNavmeshQuery, const FVector3& start, const FVector3& end, std::vector<FVector3>& paths)
{
	bool bSuccess = false;
	...
	FVector3 RcStart = UERecastHelper::Unreal2RecastPoint(start);
	FVector3 RcEnd = UERecastHelper::Unreal2RecastPoint(end);
...
bool UERecastHelper::GetRandomPointInRadius(dtNavMeshQuery* InNavmeshQuery, dtQueryFilter* InQueryFilter, const FVector3& InOrigin, const FVector3& InRedius, FVector3& OutPoint)
{
	bool bStatus = false;
	...
	FVector3 ClosestPoint;
	FVector3 RcPoint = UERecastHelper::Unreal2RecastPoint(InOrigin);
	...
	{
		OutPoint = UERecastHelper::Recast2UnrealPoint(ResultPoint);
		bStatus = true;
	}
	return bStatus;
}

void UERecastHelper::SerializedtNavMesh(const char* path, const dtNavMesh* mesh)
{
	using namespace UERecastHelper;
	if (!mesh) return;
...
dtNavMesh* UERecastHelper::DeSerializedtNavMesh(const char* path)
{
